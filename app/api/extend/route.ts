...
// Default model when the client doesn't specify one. Nano Banana — free on
// the Google AI Studio tier, cheap on OpenRouter.
...
- Do NOT change the magenta background color in any region — it must stay pure ${KEY_COLOR_HEX} everywhere outside the elements, both in the existing area and in the new area.`
    ...
    return NextResponse.json({ imageUrl: result.imageUrl, chunkInfo })
  } catch (error) {
